...
#ifdef LLAMA_EXPERTFLOW
    // ExpertFlow: notify which experts are active for this layer
    // This enables prefetching and cache management
    if (expertflow::is_expertflow_active()) {
        // Note: selected_experts and weights are ggml tensors that will be evaluated later
        // ExpertFlow will intercept the actual expert matmul operations during graph execution
        // For now, we just mark that this layer uses MoE dispatch
        // TODO: Extract expert IDs from selected_experts tensor at eval time
        // and call expertflow::get_global_backend()->prepare_experts(il, expert_ids, weights)
    }
#endif
