...
#ifdef LLAMA_EXPERTFLOW
    // Initialize ExpertFlow for MoE models
    if (model->hparams.n_expert > 0) {
        try {
            auto ef_config = expertflow::BackendConfig::defaults();
            // Auto-configure based on available VRAM (estimate from first device)
            if (!model->devices.empty()) {
                ggml_backend_dev_props props;
                ggml_backend_dev_get_props(model->devices[0], &props);
                size_t vram_bytes = props.memory_free;
                expertflow::MoeArchitecture arch;
                arch.n_layers = model->hparams.n_layer;
                arch.n_experts = model->hparams.n_expert;
                arch.n_experts_used = model->hparams.n_expert_used;
                arch.expert_weight_bytes = 0; // Will be computed from actual tensors
                arch.shared_weight_bytes = 0;
                ef_config = expertflow::BackendConfig::auto_config(vram_bytes, arch);
            }
            auto ef_backend = expertflow::ExpertFlowBackend::create(path_model, ef_config);
            if (ef_backend) {
                expertflow::set_global_backend(std::move(ef_backend));
                LLAMA_LOG_INFO("%s: ExpertFlow enabled for MoE model (%d experts, top-%d)\n",
                    __func__, model->hparams.n_expert, model->hparams.n_expert_used);
            }
        } catch (const std::exception& e) {
            LLAMA_LOG_WARN("%s: ExpertFlow initialization failed: %s\n", __func__, e.what());
        }
    }
#endif
